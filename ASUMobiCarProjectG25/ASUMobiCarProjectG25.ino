...
int get_distance(){
  int distance1 = ultrasonic.ping_cm();
  delay(1);
  int distance2 = ultrasonic.ping_cm();

  return max(distance1, distance2);

}

bool check_wall(int check_distance){
  int distance1 = get_distance();
  int distance2 = get_distance();

  if((distance1 > 0 && distance1 < check_distance) && (distance2 > 0 && distance2 < check_distance)){
    return true;
  }
  else{
    return false;
  ...
  if(input == 'W'){
    while(input != 'w'){
      ...
      if(check_wall(10)){
        ...
        else if((R == line && M == line && L == line) || (R == no_line && M == line && L == no_line)){forward(65);}       
    }
  }
